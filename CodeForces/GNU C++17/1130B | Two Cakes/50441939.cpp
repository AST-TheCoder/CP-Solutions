...
int main()
{
    ...
    ll n,i,p=1,q=1,ans=0;
    cin>>n;
    for(i=0;i<2*n;i++){
        ll x;
        cin>>x;
        if(a[x].fr==0) a[x].fr=i+1;
        else a[x].se=i+1;
    }
 
    for(i=1;i<=n;i++){
        if(abs(a[i].fr-p)+abs(a[i].se-q)<=abs(a[i].fr-q)+abs(a[i].se-p)) ans+=abs(a[i].fr-p)+abs(a[i].se-q);
        else ans+=abs(a[i].fr-q)+abs(a[i].se-p);
        p=a[i].fr;
        q=a[i].se;
    }
