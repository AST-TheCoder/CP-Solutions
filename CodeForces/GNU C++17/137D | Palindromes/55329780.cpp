...
int main()
{
    ...
    ll dp[n+1][p+1],par[n+1][p+1],idx,cng=maxi;
    mem(dp,-1);
    ...
    for(j=0,dp[0][0]=0;j<p;j++){
        for(i=j;i<n;i++){
            for(ll len=1;i+len<=n;len++){
                ll x=dp[i][j]+c[i][i+len-1];
                if(dp[i+len][j+1]>x){
                    dp[i+len][j+1]=x;
                    par[i+len][j+1]=len;
                }
    ...
    for(k=idx,i=n-1;k>0;k--){
        ll len=par[i+1][k],x=len;
        ll q=(len%2?2:1);
        i=i-len;
        while(x--){
            if((x+1)>len/2) ans+=s[i+x+1];
            else{
                ans+=s[i+x+q+1];
                q+=2;
            }
