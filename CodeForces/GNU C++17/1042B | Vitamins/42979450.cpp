...
int main(int argc, char const *argv[])
{
    ...
    a["ABC"]=maxi;a["AB"]=maxi;a["AC"]=maxi;a["BC"]=maxi;a["A"]=maxi;a["B"]=maxi;a["C"]=maxi;
    ...
    if(a["A"]!=maxi && a["B"]!=maxi && a["C"]!=maxi) x[1]=a["A"]+a["B"]+a["C"];
    if(a["A"]!=maxi && a["BC"]!=maxi) x[2]=a["A"]+a["BC"];
    if(a["AB"]!=maxi && a["BC"]!=maxi) x[3]=a["AB"]+a["BC"];
    if(a["AB"]!=maxi && a["AC"]!=maxi) x[4]=a["AB"]+a["AC"];
    if(a["AB"]!=maxi && a["C"]!=maxi) x[5]=a["AB"]+a["C"];
    if(a["ABC"]!=maxi) x[6]=a["ABC"];
    if(a["AC"]!=maxi && a["B"]!=maxi) x[7]=a["AC"]+a["B"];
    if(a["AC"]!=maxi && a["BC"]!=maxi) x[8]=a["AC"]+a["BC"];
    if(a["AB"]!=maxi && a["C"]!=maxi) x[9]=a["C"]+a["AB"];
    if(a["AC"]!=maxi && a["B"]!=maxi) x[10]=a["B"]+a["AC"];
 
    ll ans=maxi;
 
    for(i=1;i<11;i++) if(x[i]) ans=min(ans,x[i]);
