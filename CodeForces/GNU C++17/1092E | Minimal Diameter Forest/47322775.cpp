...
int main()
{
    ...
    sort(all(cen),greater<pairl>());
 
    for(i=1;i<cen.size();i++){
        a[cen[0].se].pi(cen[i].se);
        a[cen[i].se].pi(cen[0].se);
    }
    for(i=1;i<=n;i++){
        vis[i]=0;
        dist[i]=0;
    ...
    BFS(1);
    cout<<cen[cen.size()-1].fr-1<<endl;
    for(i=1;i<cen.size()-1;i++){
        cout<<cen[0].se<<" "<<cen[i].se<<endl;
    }
