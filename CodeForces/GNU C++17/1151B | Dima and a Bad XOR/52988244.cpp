...
int main()
{
    ...
    for(i=0;i<n;i++){
        for(j=0;j<m;j++){
            ...
            for(u=0;u<15;u++){
                if(p&(1<<u)){
                    if(x[i][u+1]==0) x[i][u+1]=2;
                    else if(x[i][u+1]==-1) x[i][u+1]=1;
                }
                else{
                    if(x[i][u+1]==1) x[i][u+1]=2;
                    else if(x[i][u+1]==-1) x[i][u+1]=0;
                }
            }
        }
    }
    ...
    for(i=0;i<n;i++){
        for(j=0;j<m;j++){
            if(q && (a[i][j]&(1<<u)) && x[i][u+1]==2){
                cout<<j+1<<" ";
                q=0;
                break;
            }
            else if(x[i][u+1]==1 && (a[i][j]&(1<<u))){
                cout<<j+1<<" ";
                break;
            }
            else if(q==0 && (a[i][j]&(1<<u))==0 && x[i][u+1]==2){
                cout<<j+1<<" ";
                break;
            }
            else if(x[i][u+1]<=0 && !(a[i][j]&(1<<u))){
                cout<<j+1<<" ";
                break;
            }
        }
