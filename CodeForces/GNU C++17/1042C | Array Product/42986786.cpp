...
int main(int argc, char const *argv[])
{
    ...
    ll mi=-1*maxi,ze=-1,i,a[n],f=-1,minu=0,midx=-1,z=0;
    ...
    if(ze!=-1){
        ...
        if(z!=n) cout<<2<<" "<<ze<<endl;
        f=ze;
    }
    else if(minu%2){cout<<2<<" "<<midx<<endl;f=midx;}
 
    ve ans;
    ans.pi(0);
    for(i=0;i<n;i++){
        if(minu%2 && i+1==midx) continue;
        if(a[i]!=0 && i+1!=f){
            ans.pi(i+1);
        }
    }
 
    for(i=1;i<ans.size()-1;i++) cout<<1<<" "<<ans[i]<<" "<<ans[i+1]<<endl;
