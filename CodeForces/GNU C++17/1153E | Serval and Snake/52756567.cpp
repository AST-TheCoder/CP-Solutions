...
ll bs(char ch,ll x,ll s,ll f)
{
    ll m=(s+f)/2,k;
    if(ch=='c') cout<<"?"<<" "<<s<<" "<<x<<" "<<m<<" "<<x<<endl;
    else cout<<"?"<<" "<<x<<" "<<s<<" "<<x<<" "<<m<<endl;
    cin>>k;
    if(k%2){
        if(s==m) return s;
        else return bs(ch,x,s,m);
    }
    if(ch=='c') cout<<"?"<<" "<<m+1<<" "<<x<<" "<<f<<" "<<x<<endl;
    else cout<<"?"<<" "<<x<<" "<<m+1<<" "<<x<<" "<<f<<endl;
    cin>>k;
    if(k%2){
        if(s==m) return f;
        else return bs(ch,x,m+1,f);
    }
}
