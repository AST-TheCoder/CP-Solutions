...
int main()
{
    ...
    ll f[n][m],stt[n][m],u[n][m],v[n][m],w[n][m],c[n][m]={0};
    mem(f,-1);
    mem(stt,-1);
    mem(u,-1);
    mem(v,-1);
    ...
    for(j=0;j<m;j++){
        ...
        for(ll k=1;k<(ll)p.size()-1;k++){
            if(p[k]<=p[k-1] && p[k]<=p[k+1]){
                ...
                c[x-p[k]][j]=1;
                if(j!=0){
                    if(stt[x-p[k]][j]==stt[x-p[k]][j-1]){
                        if(f[x-p[k]][j]==f[x-p[k]][j-1]){
                            if(u[x-p[k]][j]==u[x-p[k]][j-1]){
                                if(v[x-p[k]][j]==v[x-p[k]][j-1]){
                                    if(w[x-p[k]][j]==w[x-p[k]][j-1]){
                                        c[x-p[k]][j]+=c[x-p[k]][j-1];
                                        ans+=c[x-p[k]][j-1];
                                    }
                                }
                            }
