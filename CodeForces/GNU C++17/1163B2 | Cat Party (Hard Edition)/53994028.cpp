...
int main()
{
    ...
    i=0;j=n-1;len[0]=0;
    for(;i!=j;){
        if((len[cnt[a[j]]]+1==diff && (len[cnt[a[j]]]*cnt[a[j]]+cnt[a[j]]+1==j-i+1 || (diff-1)*cnt[a[j]]+1==j-i+1)) || (len[cnt[a[i]]]+1==diff && (len[cnt[a[i]]]*cnt[a[i]]+cnt[a[i]]+1==j-i+1 || (diff-1)*cnt[a[i]]+1==j-i+1)) || diff==j-i+1 || diff==1) break;
        len[cnt[a[j]]]--;
        cnt[a[j]]--;
        len[cnt[a[j]]]++;
        if(cnt[a[j]]==0) diff--;
