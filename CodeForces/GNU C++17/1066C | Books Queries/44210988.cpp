...
int main(int argc, char const *argv[])
{
    ...
    for(i=1;i<=n;i++){
        ...
        if(p=='R') rights.pi(x),ch[x]=1,pos[x]=rights.size();
        else if(p=='L') lefts.pi(x),ch[x]=-1,pos[x]=lefts.size();
 
        else{
 
                //cout<<pos[x]<<" "<<ch[x]<<endl;
 
            if(ch[x]==1){
                a=rights.size()-pos[x];
                b=pos[x]-1+lefts.size();
                cout<<min(a,b)<<endl;
 
                /*ll u=pos[x];
                rights.erase(rights.begin()+u-1);
                ...
                a=lefts.size()-pos[x];
                b=pos[x]-1+rights.size();
                cout<<min(a,b)<<endl;
 
                /*ll u=pos[x];
                lefts.erase(lefts.begin()+u-1);
