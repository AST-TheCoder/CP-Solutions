...
int main(int argc, char const *argv[])
{
    ...
    l[0]=d;u[0]=d;
    if(d<n-d){
        for(i=1;i<=d;i++) l[i]=l[i-1]-1,u[i]=u[i-1]+1;
 
        for(i=d+1;i<=n-d;i++) l[i]=l[i-1]+1,u[i]=u[i-1]+1;
 
        for(i=n-d+1;i<=n;i++) l[i]=l[i-1]+1,u[i]=u[i-1]-1;
    }
 
    else{
        for(i=1;i<=n-d;i++) l[i]=l[i-1]-1,u[i]=u[i-1]+1;
 
        for(i=n-d+1;i<=d;i++) l[i]=l[i-1]-1,u[i]=u[i-1]-1;
 
        for(i=d+1;i<=n;i++) l[i]=l[i-1]+1,u[i]=u[i-1]-1;
    }
 
    //for(i=0;i<=n;i++) cout<<i<<" "<<l[i]<<" "<<u[i]<<endl;
