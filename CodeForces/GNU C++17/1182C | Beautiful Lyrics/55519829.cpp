...
int main()
{
    ...
    for(i=0;i<an;i++){
        ...
        for(j=0;j<x[i].size();j++){
            if(j<x[i].size()-1 && x[i][j].fr==x[i][j+1].fr){
                ans[q].se=x[i][j].se;
                ans[q+1].se=x[i][j+1].se;
                q+=2;
                j++;
            }
            else{
                chk.pi(mp(i,x[i][j].se));
                if(chk.size()==2){
                    ans[p].fr=chk[0].se;
                    ans[p+1].fr=chk[1].se;
                    p+=2;
    ...
    for(i=0;i<p;i++){
        cout<<s[ans[i].fr]<<" "<<s[ans[i].se]<<endl;
    }
