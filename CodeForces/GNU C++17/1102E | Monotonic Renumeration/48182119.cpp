...
ll n,a[an],ch[an],top=-1;
vector<pairl> x;
ve y;
 
void khoda_jane(ll idx,ll val)
{
 
    ll low=(lower_bound(all(y),a[idx])-y.begin()),high;
    //cout<<low<<endl;
    for(ll i=low;i<n;i++){
        a[x[i].se]=val;
        high=i;
        ch[x[i].se]=1;
        if(i!=n && x[i].fr!=x[i+1].fr) break;
    }
    top=max(x[high].se,top);
    for(ll i=x[low].se;i<x[high].se;i++){
        if(x[high].se<top) return;
        if(!ch[i] && a[i]!=val) khoda_jane(i,val);
    }
}
