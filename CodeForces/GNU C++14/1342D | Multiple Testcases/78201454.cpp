...
int main()
{
 ...
 ll a[n];
 for(ll i=0;i<n;i++) in1(a[i]);
 sort(a,a+n);
 ll b[k+1];
 for(ll i=1;i<=k;i++) in1(b[i]);
 
 Vector ans[n+1];
 ll i=1,sz=n,j,test=0,idx[n+5];
 for(ll ch=0;ch<=n+4;ch++) idx[ch]=1;
 for(j=sz-1;j>=0;j--){
  test=max(test,i);
  //cout<<j<<" "<<i<<" "<<ans[i].size()<<" "<<a[j]<<" "<<b[a[j]]<<endl;
  if(ans[i].size()<b[a[j]]){
   ans[i].pb(a[j]);
   idx[ans[i].size()]++;
   if(j && j<sz && a[j]!=a[j-1]) i=idx[b[a[j-1]]];
   //cout<<j<<" "<<i<<endl;
  }
  else{
   i++;
 ...
 for(i=1;i<=test;i++){
  out((ll)ans[i].size());
  for(j=0;j<ans[i].size();j++) printf(" %lli",ans[i][j]);
  printf("\n");
 }
