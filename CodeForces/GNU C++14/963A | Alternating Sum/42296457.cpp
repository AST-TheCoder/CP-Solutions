...
ll steps(ll p){
    ...
    for(j=0,i=p;i<p+k;i++,j++){
 
        ll x=bigmod(a,n-i)*bigmod(b,i);
        if(s[j]=='+') sum=(sum+x%mod)%mod;
...
int main(int argc, char const *argv[])
{
    ...
    //cout<<bigmod(b,k)<<" "<<bigmod(modinverse(a),k)<<endl;
    ll r=(bigmod(b,k)*bigmod(modinverse(a),k))%mod;
    n=(n+1)/k;
    //cout<<x%mod<<" "<<r<<" "<<n<<endl;
    ...
    ll ans=((x%mod)*(((bigmod(r,n)-1)*modinverse(r-1))%mod))%mod;
    //cout<<(bigmod(r,n)-1)*modinverse(r-1)<<endl;
