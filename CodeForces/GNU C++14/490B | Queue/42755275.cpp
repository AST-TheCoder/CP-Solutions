...
ll out[an],in[an],ex[an];
ve a[an],ans1,ans2;
 
void pushin(ll source,bool x)
{
    if(x){
        if(source) ans2.pi(source);
        if(a[source].size()) pushin(a[source][0],x);
    }
    else{
        if(source) ans1.pi(source);
        if(a[source].size()) pushin(a[source][0],x);
    }
}
...
int main(int argc, char const *argv[])
{
    ...
    for(i=0;i<min(ans1.size(),ans2.size());i++){
        if(i<ans1.size() && ans1[i]) cout<<ans1[i]<<" ";
        if(i<ans2.size() && ans2[i]) cout<<ans2[i]<<" ";
    }
