...
int main()
{
 ...
 while(t--){
  ...
  for(ll i=m;i>=1;i--){
   for(ll j=0;j<idx[i].size();j++) x.insert(idx[i][j]);
   for(ll j=0;j<idx[i].size();j++){
    ll id=idx[i][j];
    if(j+1==idx[i].size()){
     ll pos=x.order_of_key(id)+1;
     if(pos==x.size()) next_max[id]=-1;
     else next_max[id]=*x.find_by_order(pos);
    }
    else{
     next_max[id]=idx[i][j+1];
    }
   }
   for(ll j=0;i+1<=m && j<idx[i+1].size();j++) x.erase(idx[i+1][j]);
  }
