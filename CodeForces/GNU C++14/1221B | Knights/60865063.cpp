...
void built(ll i,ll j,ll c){
    if(x[i][j]!=-1) return;
    x[i][j]=c;
    if(i<=n-2){
        if(j>=2) built(i+2,j-1,c==0);
        if(j<=n-1) built(i+2,j+1,c==0);
    }
    if(i>=3){
        if(j>=2) built(i-2,j-1,c==0);
        if(j<=n-1) built(i-2,j+1,c==0);
    ...
    if(j<=n-2){
        if(i>=2) built(i-1,j+2,c==0);
        if(i<=n-1) built(i+1,j+2,c==0);
    }
    if(j>=3){
        if(i>=2) built(i-1,j-2,c==0);
        if(i<=n-1) built(i+1,j-2,c==0);
...
int main()
{
    ...
    mem(x,-1);
    //for(ll i=1;i<101;i++){for(ll j=1;j<101;j++) cout<<x[i][j]<<" ";cout<<endl;}
    ll c=1,i,j;
    cin>>n;
    for(i=1;i<=n;i++){
        for(j=1;j<=n;j++){
