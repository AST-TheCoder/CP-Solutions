...
int main()
{
    ...
    while(cnt!=n-2){
        if(cnt==0){
            for(ll i=0;i<3;i++){
                if(x[a[s][i]].size()<=2 && ans[0]!=a[s][i]){
                    ans.pb(a[s][i]);
                    for(ll j=0;j<2;j++){
                        if(x[a[s][i]][j]==s) x[a[s][i]].erase(x[a[s][i]].begin()+j);
                    }
                }
            }
            for(ll i=0;i<3;i++){
        ...
        for(ll i=0,ok=0;i<x[ans[cnt]].size();i++){
            for(ll j=0;j<x[ans[cnt+1]].size();j++){
                if(x[ans[cnt]][i]==x[ans[cnt+1]][j]){
                    ok=1;
                    s=x[ans[cnt]][i];
                    x[ans[cnt]].erase(x[ans[cnt]].begin()+i);
                    x[ans[cnt+1]].erase(x[ans[cnt+1]].begin()+j);
                    break;
                }
            }
            if(ok=1) break;
        ...
        for(ll i=0;i<3;i++){
            if(a[s][i]!=ans[cnt] && a[s][i]!=ans[cnt+1]){
                ans.pb(a[s][i]);
                for(ll j=0;j<x[a[s][i]].size();j++){
                    if(x[a[s][i]][j]==s) x[a[s][i]].erase(x[a[s][i]].begin()+j);
                }
                break;
            }
    ...
    for(ll i=0;i<n;i++) printf("%lli ",ans[i]);
