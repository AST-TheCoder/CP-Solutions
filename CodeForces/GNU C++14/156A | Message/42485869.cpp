...
int main(int argc, char const *argv[])
{
    ...
    ll n=a.size(),l=b.size(),x=1,y=0,i,j,k,m=0,yy,c;
 
    for(i=0;i<n;i++){
 
        c=0;
 
        for(j=0,k=i;j<l && k<n;j++,k++){
    ...
    ll ans1=(y-x+1-m)+(l-y+x-1);
 
    for(i=0,m=0,x=1,y=0;l-i-1>=0;i++){
 
        c=0;
 
        for(j=l-i-1,k=0;j<l && k<n && j>=0;j++,k++){
