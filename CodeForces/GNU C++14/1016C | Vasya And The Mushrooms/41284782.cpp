...
int main(int argc, char const *argv[])
{
    ...
    for(i=n-1;i>0;i--){
        ...
        b[i-1]=b[i]+a[0][i]+a[1][i];
 
    }
 
    for(i=0,j=0;i<n;i++,j++){
 
        sum+=(a[0][i]*j)+(a[1][i]*(2*n-j-1));
 
    }
//cout<<sum<<" ";
    ll ms=sum;
 
    for(i=0,j=0,k=0;i<n-2;i+=2,j+=4,k+=4){
 
        sum-=((a[0][i]*j)+(a[1][i]*(2*n-1)));
        sum-=((a[0][i+1]*(j+1))+(a[1][i+1]*(2*n-2)));
        sum+=(a[0][i]*k+a[1][i]*(k+1)+a[1][i+1]*(k+2)+a[0][i+1]*(k+3));
...
//cout<<sum<<" ";
    ...
    for(i=1,sum=a[1][0],j=2;i<n;i++,j++){
 
        sum+=(a[1][i]*j)+(a[0][i]*(2*n-j+1));
 
    }
//cout<<endl<<sum<<" ";
    ms=max(ms,sum);
 
    for(i=1,j=2,k=2;i<n-2;i+=2,j+=4,k+=4){
 
        sum-=((a[1][i]*j)+(a[0][i]*(2*n-1)));
        sum-=((a[1][i+1]*(j+1))+(a[0][i+1]*(2*n-2)));
        sum+=(a[1][i]*k+a[0][i]*(k+1)+a[0][i+1]*(k+2)+a[1][i+1]*(k+3));
