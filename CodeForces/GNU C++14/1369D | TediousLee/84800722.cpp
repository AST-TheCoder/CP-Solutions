...
ll one_child[range],leaf_node[range];
...
int main()
{
 ...
 for(ll i=2;i<range;i++){
  one_child[i]=leaf_node[i-1];
  leaf_node[i]=((one_child[i-1]*2)%mod+leaf_node[i-1])%mod;
 }
 
 for(ll i=3;i<range;i++){
  one_child[i]+=one_child[i-3];
  one_child[i]%=mod;
 }
