...
int main()
{
    ...
    ll n=s.size();
    ll o[n]={0},z[n]={0},z1[n]={0},c[2][n+1],x=0,ss=0,id=-1;
    mem(c,0);
    for(ll i=0;i<n;i++){
        if(s[i]=='0') z[i]++;
        if(i) z[i]+=z[i-1];
    ...
    for(ll i=n-1;i>=0;i--){
        ...
        if(i!=n-1) o[i]+=o[i+1];
        if(s[i]=='0') z1[i]++;
        if(i!=n-1) z1[i]+=z1[i+1];
        c[0][i]+=c[0][i+1];
        c[1][i]+=c[1][i+1];
        if(i+1!=n && c[0][i+1]==c[1][i+1]){
            c[0][i]=c[1][i]=0;
        }
        if(s[i]=='0') c[0][i]++;
        else if(c[0][i]) c[1][i]++;
    ...
    for(ll i=id;i<n;i++){
        if(s[i]=='1'){
            if(o[i]+x+z[i]==ss){
                if(c[1][i+1]>=c[0][i+1] && (i+1==n || s[i+1]=='1') && o[i]-1>=z1[i]){
                    x++;
                    s[i]='0';
                    //cout<<i<<" "<<o[i]<<" "<<z[i]<<" "<<x-1<<endl;
                }
            }
        }
    }
