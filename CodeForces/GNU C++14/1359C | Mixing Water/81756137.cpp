...
int main()
{
 ...
 while(t--){
  ...
  cin>>a>>b>>c;
  ll x;
  if(a+b-2*c) x=(b-c)/(a+b-2*c);
  else x=0;
  ll ans=2;
  double diff=abs((double)c-(((double)a+(double)b)/2.0));
  double chk;
  chk=abs((double)c-((double)(x-1.0)*a+(x-2.0)*b)/(2.0*x-3.0));
  if(chk<diff && x-2>=0){
   ans=2*x-3;
   diff=chk;
  }
  //cout<<diff<<endl;
  chk=abs((double)c-((double)x*a+(x-1.0)*b)/(2.0*x-1.0));
  if(chk<diff && x-1>=0){
   ans=2*x-1;
   diff=chk;
  }
  chk=abs((double)c-((double)(x+1.0)*a+x*b)/(2.0*x+1.0));
  if(chk<diff && x>0){
   ans=2*x+1;
   diff=chk;
  }
  chk=abs((double)c-((double)(x+2.0)*a+(x+1.0)*b)/(2.0*x+3.0));
  if(chk<diff && x+1>0){
   ans=2*x+3;
   diff=chk;
  }
