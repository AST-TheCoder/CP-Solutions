...
int main()
{
 ...
 while(t--){
  ...
  ll a[n+1],next_max[n+1],ans=0,blc_length[n+1];
  ordered_set<ll> x;
  ...
  for(ll i=m;i>=1;i--){
   for(ll j=idx[i].size()-1;j>=0;j--){
    ll id=idx[i][j];
    blc_length[id]=1;
    if(i!=m){
     if(id<idx[i+1][0]) blc_length[id]=max(blc_length[id],idx[i+1].size()+blc_length[idx[i+1][idx[i+1].size()-1]]);
     else{
      ll next_id=lower_bound(all(idx[i+1]),id)-idx[i+1].begin();
      blc_length[id]=max(blc_length[id],idx[i+1].size()-next_id+1);
     }
    }
    if(j+1!=idx[i].size()) blc_length[id]=max(blc_length[id],blc_length[idx[i][j+1]]+1);
    ans=max(ans,blc_length[id]);
 
    /*if(blc_length[id]==1) next_max[id]=-1;
    else if(i!=m && blc_length[id]==blc_length[idx[i+1][0]]+1) next_max[id]=idx[i+1][0];
    else next_max[id]=idx[i][j+1];*/
   }
  }
